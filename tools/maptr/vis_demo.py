...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        """
        # import pdb;pdb.set_trace()
        fig = plt.figure(constrained_layout=True, figsize=(16 * 0.7, 5 * 0.7), facecolor='white')
        ...
        for CAM, ax in zip(CAMS, cam_axs):
            img_path = img_path_dict[CAM]
            cam_img = Image.open(img_path)
            ax.imshow(cam_img)
            # ax.set_title(CAM)
            ax.axis('off')
        
        fright_ax = fig.add_subplot(gs[:, -1])
        pred_map_img = Image.open(map_path)
        fright_ax.imshow(pred_map_img)
        fright_ax.axis('off')
        # fright_ax.set_title('pred map')
        sample_vis_path = osp.join(sample_dir, 'SAMPLE_VIS.png')
        fig.tight_layout()
        fig.savefig(sample_vis_path, facecolor=fig.get_facecolor(), edgecolor='none', dpi=100)
        # fig.savefig(sample_vis_path, facecolor=fig.get_facecolor(), edgecolor='none', dpi=300)
        """
        cam_row_1 = ['CAM_FRONT_LEFT','CAM_FRONT','CAM_FRONT_RIGHT']
        cam_row_2 = ['CAM_BACK_LEFT','CAM_BACK','CAM_BACK_RIGHT']
        row_1_list = []
        for cam in cam_row_1:
            img_path = img_path_dict[cam]
            img =cv2.imread(img_path)
            row_1_list.append(img)
        row_2_list = []
        for cam in cam_row_2:
            img_path = img_path_dict[cam]
            img =cv2.imread(img_path)
        ...
        gt_map_img = cv2.imread(gt_polyline_map_path)
        ...
        resized_gt_map_img = cv2.putText(resized_gt_map_img, 'GT', org, font, 
                        fontScale, color, thickness, cv2.LINE_AA)
        sample_img = cv2.hconcat([resized_map_img,cams_img,resized_gt_map_img])
